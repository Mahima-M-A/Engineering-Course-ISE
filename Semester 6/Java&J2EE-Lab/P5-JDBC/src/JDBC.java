  ...
  public static void main(String[] args) {
    ...
    jb2.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) { // anonymous inner class
        ...
        float climit = Float.parseFloat(txt4.getText());
        int repno = Integer.parseInt(txt5.getText());
        try {
          Class.forName("com.mysql.jdbc.Driver"); // registers and loads the driver.
          Connection conn = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/purchase", "root", "abcd");
          if (conn != null) {
    ...
    jb1.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        float com = Float.parseFloat(tx4.getText());
        float rate = Float.parseFloat(tx5.getText());
        ...
        try {
          ...
          if (conn != null) {
            ...
            stmt.executeUpdate(
                "insert into Representative values(" + repno + ",'" + rname + "','" + state + "'," + com + "," + rate + ");");
          }
        } catch (Exception ex) {
          ex.printStackTrace();
        }
        ...
        try {
          ...
          if (conn != null) {
            ...
            ResultSet rs = stmt.executeQuery(
                "select * from Representative where repno in ( select repno from Customer where credLimit>15000);");
            String s = "";
            ta.setText("");
            while (rs.next()) {
              s += rs.getInt(1) + " ";
